# Story: Create statement upload route handler
...
As a product builder of LFAS, I want `Create statement upload route handler` so that the system can progress toward trusted, private financial analysis.

## Acceptance Criteria

- [ ] PDF upload supported
- [ ] Route handler returns statement id
...
- Start with `apps/web/app/api/statements/upload/route.ts` or a colocated server action.
