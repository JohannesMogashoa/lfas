# Story: Detect Discovery Bank and TymeBank formats

## Parent
- Epic: MVP-E03 — PDF Extraction
- Feature: MVP-E03-F03 — Bank and Statement Detection

## Release / Milestone
MVP

## User Story
As a product builder of LFAS, I want `Detect Discovery Bank and TymeBank formats` so that the system can progress toward trusted, private financial analysis.

## Acceptance Criteria
- [ ] Newer bank formats have initial support

## Implementation Tasks
- [ ] Discovery markers
- [ ] TymeBank markers
- [ ] Fallback generic

## Engineering Notes
- Prefer deterministic business rules before AI.
- Keep raw bank statement data isolated behind storage and privacy boundaries.
- Add tests for parsing, validation, scoring, or security behavior where applicable.
- Update documentation when behavior or contracts are introduced.

## Dependencies
None

## Suggested Labels
release:mvp, type:story, area:parser, priority:high
