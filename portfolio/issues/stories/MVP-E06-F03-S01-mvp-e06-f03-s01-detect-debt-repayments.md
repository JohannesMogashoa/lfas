# Story: Detect debt repayments

## Parent
- Epic: MVP-E06 — Financial Engine
- Feature: MVP-E06-F03 — Debt and Savings Analysis

## Release / Milestone
MVP

## User Story
As a product builder of LFAS, I want `Detect debt repayments` so that the system can progress toward trusted, private financial analysis.

## Acceptance Criteria
- [ ] Debt repayments grouped

## Implementation Tasks
- [ ] Credit provider merchants
- [ ] Debit orders
- [ ] Loan keywords

## Engineering Notes
- Prefer deterministic business rules before AI.
- Keep raw bank statement data isolated behind storage and privacy boundaries.
- Add tests for parsing, validation, scoring, or security behavior where applicable.
- Update documentation when behavior or contracts are introduced.

## Dependencies
MVP-E06-F02-S04

## Suggested Labels
release:mvp, type:story, area:financial-engine, priority:medium
