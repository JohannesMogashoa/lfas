...
echo "Creating issues from portfolio/portfolio.json..."
...
create_issue() {
  ...
  local full_body
  full_body="$(cat "$ROOT_DIR/$body_file")"
...
for type in epic feature story; do
  jq -c --arg type "$type" '.records[] | select(.type==$type)' "$ROOT_DIR/portfolio/portfolio.json" | while read -r item; do
